package com.jr.cloud.authorization.util;

import io.jsonwebtoken.Claims;
import io.jsonwebtoken.JwtBuilder;
import io.jsonwebtoken.Jwts;
import io.jsonwebtoken.SignatureAlgorithm;
import net.sf.json.JSONObject;

import javax.crypto.spec.SecretKeySpec;
import javax.xml.bind.DatatypeConverter;
import java.security.Key;
import java.util.Date;

/**
 * Created by Administrator on 2018/1/13.
 */
public class JwtUtil {

    private static final String keyValue = "keyRoot123";

    private static  final String issuer = "jr.cloud.server";

    private static final long TTLMillis = 1800;

    /**
     * 签发JWT
     * @return
     * @throws Exception
     */
    public static String createJWT(String name, String userId) {
        SignatureAlgorithm signatureAlgorithm = SignatureAlgorithm.HS256;
        long nowMillis = System.currentTimeMillis();
        Date now = new Date(nowMillis);
        //生成签名密钥 就是一个base64加密后的字符串？
        byte[] apiKeySecretBytes = DatatypeConverter.parseBase64Binary(keyValue);
        Key signingKey = new SecretKeySpec(apiKeySecretBytes, signatureAlgorithm.getJcaName());
        JSONObject jsonObject = new JSONObject();
        jsonObject.put("userName", name);
        jsonObject.put("userLoginName", userId);
        //添加构成JWT的参数
        JwtBuilder builder = Jwts.builder().setHeaderParam("typ", "JWT")
                .setIssuedAt(now) //创建时间
                .setSubject(jsonObject.toString()) //主题，也差不多是个人的一些信息
                .setIssuer(issuer) //发送者
                .setAudience(name) //接收者
                .signWith(signatureAlgorithm, signingKey); //估计是第三段密钥
        //添加Token过期时间,不添加，采用redis中缓存记录过期来设置redis过期时间
//        if (TTLMillis >= 0) {
//            //过期时间
//            long expMillis = nowMillis + TTLMillis;
//            //现在是什么时间
//            Date exp = new Date(expMillis);
//            //系统时间之前的token都是不可以被承认的
//            builder.setExpiration(exp).setNotBefore(now);
//        }
        //生成JWT
        return builder.compact();
    }

    /**
     * 验证JWT
     *
     * @param jwtStr
     * @return
     */
    public static Claims validateJWT(String jwtStr) {
        try {
            Claims claims = Jwts.parser()
                    .setSigningKey(DatatypeConverter.parseBase64Binary(keyValue))
                    .parseClaimsJws(jwtStr).getBody();
            return claims;
        } catch (Exception ex) {
            return null;
        }
    }

}
